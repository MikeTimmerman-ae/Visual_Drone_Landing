...
def create_pad():
    ...
    # Link parameters
    link_Masses = [1, 1, 1]  # The mass for the pad and cross pieces
    linkCollisionShapeIndices = [padColBoxId, crossColBoxId, crossColBoxId2]  # Collision shapes
    linkVisualShapeIndices = [padVisualShapeId, crossVisualShapeId, crossVisualShapeId2]  # Visual shapes
    linkPositions = [[0, 0, pad_thickness], [0, 0, 5*pad_thickness], [0, 0, 5*pad_thickness]]  # Positioning of the parts
    linkOrientations = [[0, 0, 0, 1], [0, 0, 0, 1], [0, 0, 0, 1]]  # Orientations (no rotation)
    linkInertialFramePositions = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]  # Inertial frame positions
    linkInertialFrameOrientations = [[0, 0, 0, 1], [0, 0, 0, 1], [0, 0, 0, 1]]  # Inertial frame orientations
    indices = [0, 0, 0]  # Parent indices (all linked to the base)
    jointTypes = [p.JOINT_FIXED, p.JOINT_FIXED, p.JOINT_FIXED]  # Fixed joints (no movement between parts)
    axis = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]  # No axis for fixed joints
